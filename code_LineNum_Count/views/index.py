...
def user_list():
    ...
    # cursor.close()
    # conn.close()
    data_list = helper.fetch_all("SELECT id,username,nickname FROM userinfo",[])
...
def detail(nid):
    ...
    # cursor.close()
    # conn.close()
    record_list = helper.fetch_all("SELECT id,line,ctime FROM record where userid=%s",(nid,))
...
def upload():
    ...
    file_obj = request.files.get('code')            # 拿到的是werkzeug内存对象.filename .stream

    # 1. 检查上传文件后缀名
    name_ext = file_obj.filename.rsplit('.',maxsplit=1)
    if len(name_ext) != 2:      # 文件没有后缀
        return "请上传zip压缩文件"
    if name_ext[1] != 'zip':
        return "请上传zip压缩文件"
    ...
    file_path = os.path.join("files",file_obj.filename)         # 路径拼接
    ...
    # 2+3, 接收用户上传文件，并直接解压到指定目录
    import shutil
    target_path = os.path.join('files',str(uuid.uuid4()))
    shutil._unpack_zipfile(file_obj.stream,target_path)

    # 4. 遍历某目录下的所有文件
    # for item in os.listdir(target_path):      #　listdir只能拿当前目录下的所有文件，walk递归查找所有目录下的文件夹和文件
    #     print(item)
    total_num = 0
    for base_path,folder_list,file_list in os.walk(target_path):    # 当前目录路径、文件夹、文件
        for file_name in file_list:
            file_path = os.path.join(base_path,file_name)       # 拼接文件的路径
            ...
            if file_ext[1] != 'py':
                continue
            file_num = 0
            with open(file_path,'rb') as f: # 读即可
                for line in f:
                    line = line.strip()
                    if not line:        # 空行
                        continue
    ...
    # 获取当前时间
    import datetime
    ctime = datetime.date.today()           #　每天限制一次上传，date方便限制
    ...
    # cursor.close()
    # conn.close()
    data = helper.fetch_one("select id from record where ctime=%s and userid=%s",(ctime,session['user_info']['id']))
    if data:
        return "今天已经上传"
    ...
    helper.insert("insert into record(line,ctime,userid)values(%s,%s,%s)",(total_num,ctime,session['user_info']['id']))
